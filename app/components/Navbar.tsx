          ...
          >
            {navItems.map((item) => (
              <Link
                key={item}
                href={`#${item}`}
                className={`relative group ${activeSection === item ? 'text-accent' : 'text-textLight'}`}
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="fixed top-[72px] left-1/2 -translate-x-1/2 w-[95%] z-40 bg-primary/70 backdrop-blur-xl md:hidden rounded-xl"
          >
